...
public class InputFile
{
	...
	/**
	 * Returns the string formed by skipping until we find whitespace or a RESERVED_TOKEN "[]()" or
	 * EOF
	 */
	private String skipUntilWhiteSpaceOrToken()
	{
		String ret = "";

		while (!WHITESPACE.contains(sym) && !RESERVED_TOKENS.contains(sym) && !eof())
		{
			ret += sym;
			next();
		}
	...
	/**
	 * Eats two bytes of binary data as an integer
	 */
	private short readTwoByteInteger()
	{
		char b0 = sym;
		next();
		char b1 = sym;
	...
	/**
	 * eats four bytes of binary data as an integer
	 */
	private int readFourByteInteger()
	{
		char b0 = sym;
		next();
		char b1 = sym;
