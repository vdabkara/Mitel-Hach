...
{
	/**
	 * The length maximum for this table when committed to a database.
	 */
	private static final int												DB_LENGTH_MAX		=
			1024;

	/**
	 * The commit modulus for rows.
	 */
	private static final int												COMMIT_YKEY_MOD		=
			64;
	/**
	 * If we read a TSV / CSV, it will throw an UnsupportedOperationException on malformed input.
	 */
	private static final boolean											HARD_READ_FAILURE	=
			true;
	/**
	 * The threshold for failed table reads. Reading a TSV / CSV will throw an exception if more
	 * than this ratio of lines have malformed input.
	 */
	private static final double												MAX_FAILURE_RATE	=
			0.25;
	...
	{
		// System.out.println("Creating table and writing Excel file");
		// Table t = new Table();
		// t.add("Area", "CA", 1);
		// t.add("Rain", "CA", 2);
		// t.add("Area", "AK", "blah");
		...
		// t.add("Rain", "TX", 1.342);
		//
		// String outputFileName = "data\\table\\excelTest.xls";
		// t.writeExcel(outputFileName);
		// System.out.println("output at " + outputFileName);

		// just read the following table
		String fileName =
				"C:/Users/Ray/Desktop/Estimator/Input/rm_Mitel - KA Connector Enhancements.xls";
		Table t = Table.readFromExcel(fileName, false);
	...
	/**
	 * Reads a table from the given input file given the specified delimiter. Here is the policy for
	 * handing ragged data. Throws an exception if the file does not exist.
	 * 
	 * If the percentage of lines with an incorrect number of columns is > 0.25, an exception is
	 * thrown. The table is too ragged to get a lot of data.
	 * 
	 * It will trim quotes off any xkey column name.
	 */
	private void read2(String fileName)
	{
		...
		// fileName = "C:\\temp\\okrightnow_installer.log";
		Vector<String> lines = FileUtility.readLines(fileName);
		if (lines.size() == 0)
			throw new UnsupportedOperationException(
					"No lines read from the file '" + fileName + "'");

		Vector<String> fields = StringUtility.tokenize(lines.get(0), delimiter);

		System.out.println("fields " + fields);

		// for (int n = 0; n < fields.size(); n++)
		// log.add(" f(" + n + ") = '" + fields.get(n) + "'");

		// log.add(fields.size() + " fields");
		// log.add(lines.size() - 1 + " records");

		int failedLines = 0;
		int attemptedLines = 0;

		String fileNameOnly = FileUtility.getFileNameOnlyWithoutExtension(fileName);
		String pk = "read *sv " + fileNameOnly;
		...
		for (int lineNumber = 1; lineNumber < lines.size(); lineNumber++)
		{
			...
			Vector<String> values = StringUtility.tokenize(line, delimiter, includeEmpty, trim);

			// System.out.println("values size = " + values.size());
			// System.out.println("fields size = " + fields.size());

			// remove the ykey if it exists
			if (values.size() > fields.size())
				values.remove(values.size() - 1);

			// log.add("line " + lineNumber + " '" + line + "'");
			// log.add(values.size() + " values");

			// for (int n = 0; n < values.size(); n++)
			// log.add(" v(" + n + ") = '" + values.get(n) + "'");
			// KeyboardUtility.readLine();

			// if we don't have the right number of fields
			// or one less, due to tokenize not returning the last field
			attemptedLines++;
			if (values.size() != fields.size())
			{
				String msg = "Line '" + line + "'\n";
				msg += "has an incorrect number of field values.\n";
				msg += "Expected " + fields.size() + ", has " + values.size() + ".\n";

				int max = fields.size() > values.size() ? fields.size() : values.size();
				for (int i = 0; i < max; i++)
				{
					String field = i < fields.size() ? fields.get(i) : "DNE";
					String value = i < values.size() ? values.get(i) : "DNE";
					msg += "  f" + i + " ->" + field + " = '" + value + "'\n";
				}
				msg += "Is the table ragged?\n";
				msg += "File " + fileName + "\n";
			...
			{
				String xkey = fields.get(fieldIndex);
				xkey = StringUtility.trimQuotes(xkey);
				String ykey = "entry_" + (lineNumber - 1);
				String data = values.get(fieldIndex);

				add(xkey, ykey, data);
			}
		}

		double failureRate = (double) failedLines / attemptedLines;

		if (failureRate > MAX_FAILURE_RATE)
		{
			String msg = "";
			msg += "Failure rate is too high for reading table\n";
			msg += "file " + fileName + "\n";
			msg += "failure rate " + failureRate;
