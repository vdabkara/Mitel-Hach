...
//			String sql="SELECT * FROM IM_ATTACHMENT_"+channelRefKey.toUpperCase().trim()+" WHERE ATTACHMENT_TYPE  IN ('INLINE_LINKS','INLINE_LINKS_OKM')";
			String sql="SELECT * FROM IM_ATTACHMENT_"+channelRefKey.toUpperCase().trim()+" WHERE ATTACHMENT_TYPE  IN ('INLINE_IMAGES') AND REC_CREATION_TMSTP>'2022-05-27'";
//			String sql="SELECT * FROM IM_ATTACHMENT_"+channelRefKey.toUpperCase().trim()+" WHERE ATTACHMENT_TYPE  IN ('DOCUMENT') AND REC_CREATION_TMSTP>'2022-05-27'";
...
//					try
//					{
//						sql = "SELECT CDN_URL from hach.im_all_attachments where LOWER(name)=? and document_id=? and locale =? and channel = ?";
//						pstmt = conn.prepareStatement(sql);
//						pstmt.setString(1, details.getAttachmentName().toLowerCase());
	...
	private static void printReports(List<AttachmentDetails> attachmentsList)
	{
		try
		{
			...
			if(null!=attachmentsList && attachmentsList.size()>0)
			{
				...
				String fName = "/ATTACHMENT_REPORT_ALL_INLINE_IMAGES.xlsx";
